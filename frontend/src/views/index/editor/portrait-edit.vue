<template>
  <div class="portrait-edit">
    ...
    <div class="container">
      ...
      <div class="content">
        <img v-if="image_edited" :src="image_edited" alt="Portrait" class="image" />
...
import { bigeyeApi , thinApi, smoothApi, whitenApi} from '@/api/index/operations';
import { createApi } from '@/api/index/moment';
export default {
  name: 'portrait-edit',
  components: {
    Footer,
  ...
  methods: {
    handleAction(action) {
      if(action == '大眼')
      {
        ...
        bigeyeApi({
          ...
          this.image_edited = res.data.image;
        }).catch(err => {
    ...
    applyChanges() {
      this.image_original = this.image_edited;
    },
    submitText() {
      return new Promise((resolve, reject) => {
        const formData = new FormData()
        formData.append('caption', this.textInput)
